...
export const convert = (app, fileIndex) => {
    ...
    app.iterateRegions((region) => {
        ...
        if (region.data.speaker.length > 1) {
            ...
            data.push(`SPEAKER <NA> 1 ${region.start.toFixed(3)} ${(region.end.toFixed(3) - region.start.toFixed(3)).toFixed(3)} <NA> <NA> ${self.formatSpeaker(region.data.speaker)} <NA> <NA>`)
        }
    }, fileIndex, true);
